...
#define __FBGC_CLASS_VARIABLE_MAP_INITIAL_SIZE 1
#define __FBGC_CLASS_VARIABLE_MAP_INITIAL_REHASH_LOAD_FACTOR 100

void initialize_static_fbgc_class_object(struct fbgc_class_object * clso){
	clso->base.type = CLASS;
	clso->code = NULL;
	clso->variables = new_fbgc_map_object(__FBGC_CLASS_VARIABLE_MAP_INITIAL_SIZE,__FBGC_CLASS_VARIABLE_MAP_INITIAL_REHASH_LOAD_FACTOR);
}
...
struct fbgc_object * new_fbgc_class_object(){
	...
	initialize_static_fbgc_class_object(o);
...
static struct fbgc_object * get_overloaded_member(struct fbgc_object * o, const char * str){
	char buf[10]={'\0'}; //Overloadble function names are always short
