...
static struct fbgc_object * fbgc_file_fread(struct fbgc_object ** arg,int argc){
	if(argc == 1){
		struct fbgc_cstruct_object * so = cast_fbgc_object_as_cstruct(arg[0]);
...
static struct fbgc_object * fbgc_file_fprint(struct fbgc_object ** arg,int argc){
	if(argc == 2){
		struct fbgc_cstruct_object * so = cast_fbgc_object_as_cstruct(arg[0]);
...
static struct fbgc_object * fbgc_file_freadline(struct fbgc_object ** arg,int argc){
	if(argc == 2){
		struct fbgc_cstruct_object * so = cast_fbgc_object_as_cstruct(arg[0]);
		if(arg[1]->type == INT){
			struct file_struct * fs = (struct file_struct *) so->cstruct; 
...
static struct fbgc_object * fbgc_file_fclose(struct fbgc_object ** arg,int argc){
	if(argc == 1){
		struct fbgc_cstruct_object * so = cast_fbgc_object_as_cstruct(arg[0]);
...
static struct fbgc_object_method _fbgc_file_cstruct_methods = {
	...
	.method = {
		...
		{.name = "freadline", .function = &fbgc_file_fclose},
		{.name = "fclose", .function = &fbgc_file_freadline},
	}
};
...
static struct fbgc_object_method _fbgc_file_module_methods = {
	.len = 1,
	.method = {
...
const struct fbgc_object_property_holder _fbgc_file_module_property_holder = {
	.bits = 
	_BIT_METHODS
	,
	.properties ={
		{.methods = &_fbgc_file_module_methods},
...
const struct _fbgc_cmodule _fbgc_file_module = 
{
	.initializer = {.name = "file", .function = NULL},
	.properties = &_fbgc_file_module_property_holder
};